...
Page({
  ...
  onLoad: function (options) {
    ...
    contactsCollection.doc(contactId).get({
      success: function (res) {
        ...
        self.setData({
          contact: res.data // 将联系人数据保存到 data 中
        });
      },
      fail: function (err) {
  ...
  onNoteInput: function (e) {
    this.setData({
      'contact.note': e.detail.value
    });
  },

  // 动态更新额外信息的类型
  onExtraInfoTypeInput: function (e) {
    const { index } = e.currentTarget.dataset;
    const value = e.detail.value;
    const extraInfoList = this.data.contact.extraInfoList || [];
    extraInfoList[index].type = value;
    this.setData({
      'contact.extraInfoList': extraInfoList
    });
  },

  // 动态更新额外信息的内容
  onExtraInfoContentInput: function (e) {
    const { index } = e.currentTarget.dataset;
    const value = e.detail.value;
    const extraInfoList = this.data.contact.extraInfoList || [];
    extraInfoList[index].content = value;
    this.setData({
      'contact.extraInfoList': extraInfoList
    });
  },
  ...
  addExtraInfo: function () {
    const extraInfoList = this.data.contact.extraInfoList || [];
    extraInfoList.push({ type: '', content: '' }); // 添加一个空的额外信息
    this.setData({
      'contact.extraInfoList': extraInfoList
    });
  },
  ...
  onSaveContact: function () {
    ...
    contactsCollection.doc(self.data.contactId).update({
      data: {
        ...
        extraInfoList: updatedContact.extraInfoList || [] // 保存额外信息
      },
      success: function () {
        wx.showToast({
          title: '联系人已更新',
